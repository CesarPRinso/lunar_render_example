...
def create_environment(enviroment):
    # Create simulated environment
    # environment = gym.make(enviroment, render_mode="human")
    environment = gym.make(enviroment)
...
class Agent:
    ...
    def train_model(self, env, num_episodes, graph):
        environment, number_of_observations, number_of_actions = create_environment("LunarLander-v2")
        env = environment
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                score += reward
                self.store_tuple(state, action, reward, observation, done)
                state = observation
                self.train()
            scores.append(score)
            obj.append(goal)
